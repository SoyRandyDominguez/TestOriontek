...
namespace TestOriontec.Tests
{
    public abstract class TestOriontecTestBase : AbpIntegratedTestBase<TestOriontecTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected TestOriontecTestBase()
        {
        ...
        protected void UsingDbContext(Action<TestOriontecDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TestOriontecDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TestOriontecDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TestOriontecDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TestOriontecDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TestOriontecDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TestOriontecDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TestOriontecDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TestOriontecDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TestOriontecDbContext, Task<T>> func)
        {
            T result;
